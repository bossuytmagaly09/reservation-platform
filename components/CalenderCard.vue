...
const props = defineProps({
  filteredResourceId: [Number, String],  // Optioneel: filter op resource
  filteredResourceName: String            // Optioneel: naam van resource
})
...
// --- 2. EVENTS MAPPEN MET FILTERING ---
const calendarEvents = computed(() => {
  if (!boardStore.reservations || !Array.isArray(boardStore.reservations)) {
    return []
  }

  let events = boardStore.reservations.map(res => {
  ...
  // Filter op resource als gefilterd
  if (props.filteredResourceId) {
    events = events.filter(e => e.resourceId === parseInt(props.filteredResourceId))
  }
...
<template>
  <div class="relative">
    <div class="bg-slate-800 p-5 rounded-2xl shadow-xl border border-slate-700">
      <!-- Header met resource filter info -->
      <div v-if="filteredResourceId" class="mb-4 pb-3 border-b border-slate-700/50 flex items-center justify-between">
        <div class="flex items-center gap-2">
          <svg class="w-4 h-4 text-cyan-400" fill="currentColor" viewBox="0 0 256 256">
            <path d="M224,128a96,96,0,1,1-96-96A96,96,0,0,1,224,128Z"></path>
          </svg>
          <span class="text-sm font-semibold text-cyan-300">Gefilterd op: <span class="text-white">{{ filteredResourceName }}</span></span>
        </div>
      </div>
