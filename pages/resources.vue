  ...
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-end px-1">
      <div>
        ...
        <p class="text-slate-400 text-sm mt-1">Beheer hier al je beschikbare middelen.</p>
      </div>
    ...
    <div class="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 shadow-xl min-h-[400px]">
      ...
      <div v-if="loading" class="text-center py-20 text-slate-400">⏳ Resources laden...</div>
      <div v-else-if="error" class="text-center py-20 text-red-500">❌ {{ error }}</div>
      <div v-else-if="resources.length === 0" class="text-center py-20 text-slate-500">Geen resources gevonden.</div>
...
const showCreateModal = ref(false) // State voor de popup
...
const handleSuccess = () => {
  ...
  fetchResources() // Ververs de lijst na toevoegen
}
